...
class PolyTensorRegression(nn.Module):
    ...
    def predict_with_polynomial(self, X):
        """Predict using learned polynomial expression"""
        if not isinstance(X, torch.Tensor):
            X_tensor = torch.tensor(X, dtype=torch.float32, device=self.device)
        else:
            X_tensor = X.to(self.device)
        ...
        predictions = torch.zeros_like(x_values)
        
        # Add constant term
        beta_value = self.beta.item()
        if abs(beta_value) > threshold:
            predictions += beta_value
        ...
        for i, c in enumerate(self.C):
            ...
            if abs(c_value) > threshold:
                power = i + 1
                predictions += c_value * (x_values ** power)
